...
import static com.gyx.mybitcoinwallet.btc.BTCUtils.decodeBase58;
...
public final class Address {
    ...
    public static final int PUBLIC_KEY_TO_ADDRESS_LEGACY = 1;//一般验证
    static final int PUBLIC_KEY_TO_ADDRESS_P2WKH = 2;//隔离验证
    ...
    Address(boolean testNet, Transaction.Script.WitnessProgram witnessProgram) throws BitcoinException {
        ...
        keyhashType = TYPE_NONE;
        hash160 = null;
        addressString = Bech32.encodeSegwitAddress(testNet ? "bc" : "tb", witnessProgram.version, witnessProgram.program);
    }
    ...
    static String publicKeyToP2wkhAddress(boolean testNet, byte[] publicKey) {
        ...
        try {
            return Bech32.encodeSegwitAddress(testNet ? "tb" : "bc", 0, BTCUtils.sha256ripemd160(publicKey));
